...
Public Class RepVentaHVI
    ...
    '    oSheet.Range("T10").Value = "KILOS"
    '    ' Formato Columnas
    '    oSheet.Range("A10:T10").Font.FontStyle = "Bold"
    ...
    '            Dim t As String = "T" + col.ToString + ""

    '            oSheet.Range(a).Value = TablaExporta.Rows(ii).Item("BaleID").ToString
    '            oSheet.Range(b).Value = TablaExporta.Rows(ii).Item("Descripcion").ToString
    ...
    '            oSheet.Range(t).Value = TablaExporta.Rows(ii).Item("Kilos").ToString
    ...
    '            oSheet.Range(t).NumberFormat = "#0.00"
    ...
    Private Sub ExportarAExcelToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ExportarAExcelToolStripMenuItem.Click
        ...
        If Not Directory.Exists(System.Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\CalculaCotton Export ventas excel" & "\" & BaseDeDatos) Then
            Directory.CreateDirectory(System.Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\CalculaCotton Export ventas excel" & "\" & BaseDeDatos)
        End If
        Dim strFN As String = System.Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\CalculaCotton Export ventas excel" & "\" & BaseDeDatos & "\Detalle_hvi_ID_Venta_" & IdVenta & ".xlsx"   'must have ".xlsx" extension

        'Standard code for filling a DataTable from SQL Server
        Dim strSym As String = "Detalle HVI"
        Dim MyTable As New DataTable

        EntidadReportes.Reporte = Reporte.ReporteVentaPacasExcel
        EntidadReportes.IdVenta = IdVenta
        NegocioReportes.Consultar(EntidadReportes)
