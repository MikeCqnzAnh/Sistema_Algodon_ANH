...
Public Class Produccion
                ...
                If TbFolioCIA.Text <> EtiquetaEscaneada Then
                    ...
                    If opc = DialogResult.Yes Then
                        If ConsultarPacaExistente(EtiquetaEscaneada, CbPlantaOrigen.SelectedValue) = 1 Then
                            ...
                            Dim kilos As Decimal = Convert.ToDecimal(TbKilos.Text)
                            Dim EntidadProduccion As New Capa_Entidad.Produccion
                            Dim NegocioProduccion As New Capa_Negocio.Produccion
                            ...
                            EntidadProduccion.BaleID = TbFolioCIA.Text
                            EntidadProduccion.Tipo = CbTipoProducto.Text
                            EntidadProduccion.Kilos = kilos
                            EntidadProduccion.Libras = kilos * 2.2046
                            EntidadProduccion.Quintales = kilos / 46.02
    ...
    Private Sub PropiedadesDGV()
        ...
        DgvPacas.Columns("BaleID").ReadOnly = True
    ...
    Private Sub BtCerrarProduccion_Click(sender As Object, e As EventArgs) Handles BtCerrarProduccion.Click
        ...
        BtCerrarProduccion.Enabled = False
    End Sub
    Private Function ConsultarPacaExistente(ByVal BaleId As Long, ByVal IdPlantaElabora As Integer)
        Dim EntidadProduccion As New Capa_Entidad.Produccion
        Dim NegocioProduccion As New Capa_Negocio.Produccion
        Dim Tabla As New DataTable
        ...
        EntidadProduccion.BaleID = BaleId
        EntidadProduccion.IdPlantaOrigen = CbPlantaOrigen.SelectedValue
        ...
        If Tabla.Rows(0).Item("Resultado") = 1 Then
            ...
            BaleIDReturn = Tabla.Rows(0).Item("BaleID")
            Return 1
        Else
            Return 0
        End If
        ...
        Try
            If IndicadorPesoBruto <> "" Then
                If returnStr.Contains(IndicadorPesoBruto) And TextoIzq = False Then
                    ...
                    ElseIf CkLeersaco.Checked = False Then
                        ActualizacionPacaAutoSinSaco(Bruto, FechaActualizacion)
                    End If
    ...
    Private Sub ActualizaPesoModuloAutomatico(ByVal Bruto As Double, ByVal FechaActualizacion As DateTime)
        ...
        GuardarProduccion()
    End Sub
    Private Sub ActualizacionPacaAutoSinSaco(ByVal Bruto As Double, ByVal FechaActualizacion As DateTime)
        If TbIdOrdenTrabajo.Text <> "" Then
            If TbKilos.Text <> "" And TbFolioCIA.Text <> "" Then
                If ConsultarPacaExistente(TbFolioCIA.Text, CbPlantaOrigen.SelectedValue) = 1 Then
                    'MsgBox("Folio existente para esta planta, verificar", MsgBoxStyle.OkOnly Or MsgBoxStyle.Exclamation, "Aviso")
        ...
        If opc = DialogResult.Yes Then
            Try
                For i As Integer = 0 To DgvPacas.Rows.Count() - 1
                    ...
                    If c = True Then
                        GeneraRegistroBitacora(Me.Text.Clone.ToString, BtEliminarPacas.Text, DgvPacas.Rows(i).Cells("BaleID").Value, DgvPacas.Rows(i).Cells("Kilos").Value & " KGs DEL PRODUCTOR " & TbNombreProductor.Text & " CON LA ORDEN No " & TbIdOrdenTrabajo.Text & " EN PLANTA " & CbPlantaOrigen.Text)
                        EliminaPaca(DgvPacas.Rows(i).Cells("IdProduccionDetalle").Value, TbIdOrdenTrabajo.Text)
                    End If
                Next
            Catch ex As Exception
                MsgBox(ex)
    ...
    End Sub
    Private Sub EliminaPaca(ByVal idproddet As Integer, ByVal IdOrdenTrabajo As Integer)
        Dim EntidadProduccion As New Capa_Entidad.Produccion
        Dim NegocioProduccion As New Capa_Negocio.Produccion
        Dim Tabla As New DataTable
        EntidadProduccion.Eliminar = Eliminar.EliminaPacaSeleccionada
        EntidadProduccion.IdProduccionDetalle = idproddet
