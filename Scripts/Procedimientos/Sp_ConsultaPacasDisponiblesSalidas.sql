Create Procedure Sp_ConsultaPacasDisponiblesSalidas
@Seleccionar bit = 0 ,
...
where cc.EstatusEmbarque = 1 AND CC.EstatusSalida IS NULL and cc.IdEmbarqueEncabezado = @IdEmbarqueEncabezado
...
from PaqueteEncabezado pe inner join CalculoClasificacion cc on pe.IdPaquete = cc.IdPaqueteEncabezado and pe.IdPlanta = cc.idPlantaOrigen
						  inner join Plantas Pl on cc.IdPlantaOrigen = pl.IdPlanta
						  inner join ventapacas vp on cc.IdVentaenc = vp.Idventa  
where cc.EstatusEmbarque = 1 AND CC.EstatusSalida IS NULL  and cc.IdEmbarqueEncabezado = @IdEmbarqueEncabezado and cc.NoLote = @NoLote
order by cc.IdPaqueteEncabezado, cc.BaleID
end