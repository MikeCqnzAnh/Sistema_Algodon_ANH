Create Procedure Sp_ConsultaPacasSinVender
@Valor nvarchar(max),
@IdPlanta integer,
@Clase varchar(4)
as
...
set @sql =  'SELECT Pl.Descripcion as Planta
  ...
  WHERE [IdVentaEnc] is null and cast (baleid as nvarchar)  in ('+@valor+')
...
set @sql =  'SELECT Pl.Descripcion as Planta
      ...
      ,isnull([UV],0) as UV
  FROM [CalculoClasificacion] cc inner join Plantas Pl on cc.IdPlantaOrigen = Pl.IdPlanta
  WHERE [IdVentaEnc] is null and IdPlantaOrigen = '+cast(@IdPlanta as nvarchar)+'
  ORDER BY IdPlantaOrigen, BaleID'
end
else 
begin
